...
import { BlogPost, Category, FeaturedPost, RelatedPost } from "@/types/blog";
...
const SinglePostClient = ({
  ...
  const [relatedPosts, setRelatedPosts] =
    useState<RelatedPost[]>(initialRelatedPosts);
  const [isBookmarked, setIsBookmarked] = useState(false);
  const [showShareMenu, setShowShareMenu] = useState(false);
  const [categories] = useState<Category[]>(initialCategories);
  const [featuredPosts] = useState<FeaturedPost[]>(initialFeaturedPosts);
  ...
  // Clean HTML content - preserve important attributes like href, target, rel
  const cleanHTMLContent = (html: string) => {
    if (!html) return "";

    // Only remove inline styles that might interfere with styling
    // But preserve all other attributes including href, target, rel
    let cleaned = html.replace(/style="[^"]*"/g, "");

    // Don't remove span tags completely, just clean them
    // Only remove spans that are empty or only contain styles
    cleaned = cleaned.replace(/<span[^>]*>\s*<\/span>/g, "");

    // Replace grey text color with default but keep the span
    cleaned = cleaned.replace(/color:\s*rgb\(156,\s*163,\s*175\)/g, "");
    cleaned = cleaned.replace(/color:\s*#9ca3af/g, "");

    // Remove background colors but keep the element
